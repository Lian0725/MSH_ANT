...
from qc import qc
if os.environ.get("DISPERPICKER_ALLOW_GPU") != "1":
...
class Pick(object):
    ...
    def _auto_ref_T_for_distance(self, dist):
        """Return DisperPicker's automatic phase reference column for one path."""
        index = int(
            0.6 * min(
                self.range_T[2] - 1,
                round((dist / 1.5 / 3.2 - self.range_T[0]) / self.dT),
            )
        )
        return max(0, min(self.range_T[2] - 1, index))


    def _resolve_ref_T_for_distance(self, dist, ref_T=_REF_T_UNSET):
        """Resolve phase reference column without mutating the persistent setting."""
        if ref_T is _REF_T_UNSET:
            ref_T = self.ref_T
        if ref_T is not None:
            return max(0, min(self.range_T[2] - 1, int(ref_T)))
        return self._auto_ref_T_for_distance(dist)
    ...
    def pick(self, group_image, phase_image, sta_info, snr, file_list, ct, save_result, 
        ...
        for i in range(self.batch_size):
            ...
            dist = sta_info[i, 0]
            self.ref_T = self._resolve_ref_T_for_distance(dist, ref_T=manual_ref_T)
