...
@dataclass(frozen=True)
class MemoryAvailabilityEvidence:
    platform: str
    source: str
    raw_field: str
    raw_value: int
    raw_unit: str
    bytes_per_raw_unit: int
    canonical_field: str
    canonical_value: int
    canonical_unit: str
    ...
    def __post_init__(self) -> None:
        ...
        integers = {name: getattr(self, name) for name in integer_names}
        if (
            any(
            ...
            or self.canonical_field != "mem_available_bytes"
            or self.canonical_unit != "bytes"
            or int(self.canonical_value)
            != int(self.raw_value) * int(self.bytes_per_raw_unit)
            or (
                self.platform == "Linux"
                and (
                    self.source != "/proc/meminfo"
                    or self.raw_field != "MemAvailable"
                    or self.raw_unit != "kB"
                    or int(self.bytes_per_raw_unit) != 1024
                )
            )
            or (
                self.platform != "Linux"
                and (
                    (self.source, self.raw_field)
                    not in {
                        ("os.sysconf", "SC_AVPHYS_PAGES"),
                        ("vm_stat", "Pages free"),
                    }
                    or self.raw_unit != "pages"
                )
            )
            or (
                self.source == "vm_stat"
            ...
            raise ValueError(
                "MemoryAvailabilityEvidence fields are inconsistent"
            )
...
class ProcessIdentityEvidence:
    ...
    def __post_init__(self) -> None:
        if (
            isinstance(self.pid, (bool, np.bool_))
            or not isinstance(self.pid, (int, np.integer))
            or int(self.pid) <= 0
            or not isinstance(self.platform, str)
            or not self.platform
            or not isinstance(self.source, str)
            or not self.source
            or not isinstance(self.native_start_time, str)
            or not self.native_start_time
            or (
                self.platform == "Linux"
                and (
                    self.source != "/proc/<pid>/stat:starttime"
                    or isinstance(self.starttime_ticks, (bool, np.bool_))
                    or not isinstance(
                        self.starttime_ticks,
                        (int, np.integer),
                    )
                    or int(self.starttime_ticks) <= 0
                    or self.native_start_time
                    != str(int(self.starttime_ticks))
                )
            )
            or (
                self.platform != "Linux"
                and (
                    ...
                    or self.source != "ps:lstart"
                    or self.starttime_ticks is not None
                )
            )
        ):
            raise ValueError(
                "ProcessIdentityEvidence fields are inconsistent"
            )
        object.__setattr__(self, "pid", int(self.pid))
        if self.starttime_ticks is not None:
            object.__setattr__(
                self,
                "starttime_ticks",
                int(self.starttime_ticks),
            )


def derive_pool_resource_lifecycle_id(
    *,
    process_identities: Sequence[ProcessIdentityEvidence],
    started_monotonic_s: float,
    lifecycle_nonce: str,
) -> str:
    """Derive an immutable lifecycle ID from identities, start, and nonce."""

    identities = []
    for value in process_identities:
        identity = value
        if not isinstance(identity, ProcessIdentityEvidence):
            identity = ProcessIdentityEvidence(**dict(value))
        identities.append(identity)
    started = float(started_monotonic_s)
    if (
        not identities
        or len({value.pid for value in identities}) != len(identities)
        or not np.isfinite(started)
        or not isinstance(lifecycle_nonce, str)
        or len(lifecycle_nonce) != 32
        or any(character not in "0123456789abcdef" for character in lifecycle_nonce)
    ):
        raise ValueError("pool resource lifecycle inputs are inconsistent")
    payload = {
        "process_identities": [
            {
                "pid": value.pid,
                ...
                "native_start_time": value.native_start_time,
            }
    ...
    return "stage-b-pool-" + hashlib.sha256(
        json.dumps(
            payload,
            sort_keys=True,
            separators=(",", ":"),
...
class PoolResourceEvidence:
    ...
    minimum_mem_available_observation: MemoryAvailabilityEvidence
    minimum_complete_mem_available_bytes: int
    peak_aggregate_rss_bytes: int
    peak_sampled_monotonic_s: float
    peak_rss_by_pid: Mapping[int, int]

    def __post_init__(self) -> None:
        raw_parent = self.parent_pid
        raw_workers = tuple(self.worker_pids)
        raw_samples = self.complete_sample_count
        raw_minimum = self.minimum_complete_mem_available_bytes
        raw_peak = self.peak_aggregate_rss_bytes
        parent = int(self.parent_pid)
        workers = tuple(int(value) for value in raw_workers)
        started = float(self.started_monotonic_s)
        ended = float(self.ended_monotonic_s)
        samples = int(self.complete_sample_count)
        minimum = int(self.minimum_complete_mem_available_bytes)
        peak = int(self.peak_aggregate_rss_bytes)
        peak_sampled = float(self.peak_sampled_monotonic_s)
        observation = self.minimum_mem_available_observation
        if not isinstance(observation, MemoryAvailabilityEvidence):
            try:
                observation = MemoryAvailabilityEvidence(
                    **dict(observation)
                )
            except (TypeError, ValueError) as exc:
                raise ValueError(
                    "PoolResourceEvidence fields are inconsistent"
                ) from exc
        identities = []
        try:
        ...
        raw_rss = dict(self.peak_rss_by_pid)
        try:
            rss_by_pid = {
        ...
        if (
            ...
            or self.lifecycle_id
            != derive_pool_resource_lifecycle_id(
                process_identities=identities,
                started_monotonic_s=started,
                lifecycle_nonce=self.lifecycle_nonce,
            )
            or isinstance(raw_parent, (bool, np.bool_))
            ...
            or parent in workers
            or not np.isfinite(started)
            or not np.isfinite(ended)
            or started >= ended
            or isinstance(raw_samples, (bool, np.bool_))
            or not isinstance(raw_samples, (int, np.integer))
            or samples <= 0
            or isinstance(raw_minimum, (bool, np.bool_))
            or not isinstance(raw_minimum, (int, np.integer))
            or minimum <= 0
            or minimum != observation.canonical_value
            or isinstance(raw_peak, (bool, np.bool_))
            or not isinstance(raw_peak, (int, np.integer))
            or peak <= 0
            or not np.isfinite(peak_sampled)
            or not started <= peak_sampled <= ended
            or len(rss_by_pid) != len(raw_rss)
            or any(
                isinstance(value, (bool, np.bool_))
                or not isinstance(value, (int, np.integer, str))
                for value in raw_rss
            )
            or set(rss_by_pid) != expected_pids
            or tuple(value.pid for value in identities)
            != (parent,) + workers
            or any(
                isinstance(value, (bool, np.bool_))
                or not isinstance(value, (int, np.integer))
            ...
            or sum(rss_by_pid.values()) != peak
        ):
            raise ValueError("PoolResourceEvidence fields are inconsistent")
        object.__setattr__(self, "parent_pid", parent)
        object.__setattr__(self, "worker_pids", workers)
        object.__setattr__(self, "process_identities", identities)
        object.__setattr__(self, "started_monotonic_s", started)
        object.__setattr__(self, "ended_monotonic_s", ended)
        object.__setattr__(self, "complete_sample_count", samples)
        object.__setattr__(
            self,
        ...
        object.__setattr__(self, "peak_aggregate_rss_bytes", peak)
        object.__setattr__(
            self,
            "peak_sampled_monotonic_s",
            peak_sampled,
        )
        object.__setattr__(
            self,
            "peak_rss_by_pid",
            MappingProxyType(dict(sorted(rss_by_pid.items()))),
        )


@dataclass(frozen=True)
class StageBMemoryGateMeasurement:
    lifecycle_id: str
...
class StageBBenchmarkEvidence:
    ...
    ftan_resource_evidence: PoolResourceEvidence
    fit_resource_evidence: PoolResourceEvidence
    ...
    def __post_init__(self) -> None:
        ...
        raw_counts = {name: getattr(self, name) for name in count_names}
        counts = {name: int(value) for name, value in raw_counts.items()}
        raw_ftan_pids = tuple(self.ftan_actual_worker_pids)
        raw_fit_pids = tuple(self.fit_actual_worker_pids)
        ftan_pids = tuple(int(value) for value in raw_ftan_pids)
        fit_pids = tuple(int(value) for value in raw_fit_pids)
        ftan_resource = self.ftan_resource_evidence
        if not isinstance(ftan_resource, PoolResourceEvidence):
            try:
                ftan_resource = PoolResourceEvidence(
                    **dict(ftan_resource)
                )
            except (TypeError, ValueError) as exc:
                raise ValueError(
                    "StageBBenchmarkEvidence fields are inconsistent"
                ) from exc
        fit_resource = self.fit_resource_evidence
        if not isinstance(fit_resource, PoolResourceEvidence):
            try:
                fit_resource = PoolResourceEvidence(**dict(fit_resource))
            except (TypeError, ValueError) as exc:
                raise ValueError(
                    "StageBBenchmarkEvidence fields are inconsistent"
                ) from exc
        ...
        if (
            ...
            or any(
                isinstance(value, (bool, np.bool_))
                or not isinstance(value, (int, np.integer))
                for value in raw_ftan_pids + raw_fit_pids
            )
            or any(value <= 0 for value in counts.values())
            ...
            or counts["ftan_requested_worker_count"]
            != counts["fit_requested_worker_count"]
            ...
            or ftan_resource.lifecycle_id == fit_resource.lifecycle_id
            or ftan_resource.parent_pid != counts["ftan_creator_pid"]
            or fit_resource.parent_pid != counts["fit_creator_pid"]
            or ftan_resource.worker_pids != ftan_pids
            or fit_resource.worker_pids != fit_pids
            or ftan_resource.started_monotonic_s
            != timings["ftan_pool_started_monotonic_s"]
            or ftan_resource.ended_monotonic_s
            != timings["ftan_pool_ended_monotonic_s"]
            or fit_resource.started_monotonic_s
            != timings["fit_pool_started_monotonic_s"]
            or fit_resource.ended_monotonic_s
            != timings["fit_pool_ended_monotonic_s"]
        ...
        object.__setattr__(
            self,
            "ftan_resource_evidence",
            ftan_resource,
        )
        object.__setattr__(
            self,
            "fit_resource_evidence",
            fit_resource,
        )
    ...
    def measured_peak_memory_bytes(self) -> int:
        """Compatibility alias paired with available_memory_bytes."""

        return self.memory_gate_measurement.aggregate_rss_bytes

    @property
    def absolute_max_aggregate_rss_bytes(self) -> int:
        return max(
            self.ftan_resource_evidence.peak_aggregate_rss_bytes,
            self.fit_resource_evidence.peak_aggregate_rss_bytes,
        )

    @property
    def ftan_aggregate_peak_rss_bytes(self) -> int:
        return self.ftan_resource_evidence.peak_aggregate_rss_bytes

    @property
    def fit_aggregate_peak_rss_bytes(self) -> int:
        return self.fit_resource_evidence.peak_aggregate_rss_bytes

    @property
    def _memory_gate_resource_evidence(self) -> PoolResourceEvidence:
        resources = (
            self.ftan_resource_evidence,
            self.fit_resource_evidence,
        )
        return max(
            resources,
            key=lambda evidence: (
                evidence.peak_aggregate_rss_bytes
                / evidence.minimum_complete_mem_available_bytes
            ),
        )

    @property
    def memory_gate_measurement(self) -> StageBMemoryGateMeasurement:
        resource = self._memory_gate_resource_evidence
        return StageBMemoryGateMeasurement(
            lifecycle_id=resource.lifecycle_id,
            aggregate_rss_bytes=resource.peak_aggregate_rss_bytes,
            mem_available_bytes=(
                resource.minimum_complete_mem_available_bytes
            ),
            fraction=(
                resource.peak_aggregate_rss_bytes
                / resource.minimum_complete_mem_available_bytes
            ),
        )
    @property
    def memory_gate_aggregate_rss_bytes(self) -> int:
        return self.memory_gate_measurement.aggregate_rss_bytes

    @property
    def memory_gate_mem_available_bytes(self) -> int:
        return self.memory_gate_measurement.mem_available_bytes

    @property
    def available_memory_bytes(self) -> int:
        """Compatibility alias for the same-lifecycle gate denominator."""

        return self.memory_gate_mem_available_bytes
...
def project_stage_b_budget_from_fixed_benchmark(
    ...
    *,
    ...
    memory_gate = evidence.memory_gate_measurement
    return evaluate_stage_b_budget(
        ...
        worker_count=worker_count,
        measured_peak_memory_bytes=memory_gate.aggregate_rss_bytes,
        available_memory_bytes=memory_gate.mem_available_bytes,
    )


def validate_archived_stage_b_budget(
    *,
    ...
    """Rebuild the formal budget and reject any divergent archive fields."""

    try:
        benchmark = StageBBenchmarkEvidence(**dict(benchmark_payload))
        archived = StageBBudgetResult(**dict(budget_payload))
        recomputed = project_stage_b_budget_from_fixed_benchmark(
            benchmark,
            selected_pair_count=selected_pair_count,
            maximum_measurement_class_count=maximum_measurement_class_count,
            worker_count=worker_count,
        )
    except (TypeError, ValueError) as exc:
        raise ValueError("archived Stage B budget is inconsistent") from exc
    if archived != recomputed:
        raise ValueError("archived Stage B budget is inconsistent")
    return archived
