...
from bensen_phase_ftan import (
    ...
    prepared_reference_objective_value,
    prepared_reference_objective_value_and_gradient,
...
ACCELERATION_NVIDIA_SMI_TIMEOUT_S = 10.0
...
class AccelerationSystemSnapshot:
    """Immutable raw dependency and hardware observations for Stage B."""

    python_version: str
    dependency_versions: Mapping[str, Optional[str]]
    cpu_count: int
    thread_environment: Mapping[str, Optional[str]]
    threadpool_backends: Tuple[Tuple[str, str, str, int], ...]
    numba_available: bool
    numba_compatible: bool
    numba_failure_reason: Optional[str]
    cupy_available: bool
    cupy_compatible: bool
    cupy_failure_reason: Optional[str]
    cuda_driver_version: Optional[str]
    cuda_runtime_version: Optional[str]
    gpu_name: Optional[str]
    gpu_compute_capability: Optional[str]
    gpu_total_memory_mib: Optional[int]
    cuda_visible_memory_mib: Optional[int]
    nvidia_driver_version: Optional[str] = None
    gpu_device_identifier: Optional[str] = None
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "thread_environment",
            MappingProxyType(dict(self.thread_environment)),
        )
        object.__setattr__(
            self,
            "threadpool_backends",
            tuple(tuple(row) for row in self.threadpool_backends),
        )

    @property
    def gpu_vram_mib(self) -> Optional[int]:
        """Backward-compatible alias for physical GPU memory."""

        return self.gpu_total_memory_mib

    def to_canonical_payload(self) -> Dict[str, object]:
        """Return a fresh JSON-safe snapshot suitable for archival."""

        return _acceleration_snapshot_payload(self)


@dataclass(frozen=True)
class AccelerationBackendEvidence:
    """Validated Stage B dependency lock and initial backend selection."""

    selected_ridge_backend: str
    ...
    gpu_name: Optional[str]
    gpu_device_identifier: Optional[str]
    gpu_compute_capability: Optional[str]
    gpu_total_memory_mib: Optional[int]
    cuda_visible_memory_mib: Optional[int]
    ...
    def to_canonical_payload(self) -> Dict[str, object]:
        """Return the fresh JSON-safe contract used by the lock digest."""

        return _acceleration_contract_payload(
            self,
            selected_backend=self.selected_ridge_backend,
            fallback_reason=self.fallback_reason,
        )


def _acceleration_snapshot_payload(source: object) -> Dict[str, object]:
    return {
        "schema_version": 1,
        "python_version": source.python_version,
        "dependency_versions": dict(source.dependency_versions),
        "cpu_count": source.cpu_count,
        "thread_environment": dict(source.thread_environment),
        "threadpool_backends": [
            list(row) for row in sorted(source.threadpool_backends)
        ],
        "numba_available": source.numba_available,
        "numba_compatible": source.numba_compatible,
        "numba_failure_reason": source.numba_failure_reason,
        "cupy_available": source.cupy_available,
        "cupy_compatible": source.cupy_compatible,
        "cupy_failure_reason": source.cupy_failure_reason,
        "cuda_driver_version": source.cuda_driver_version,
        "cuda_runtime_version": source.cuda_runtime_version,
        "nvidia_driver_version": source.nvidia_driver_version,
        "gpu_name": source.gpu_name,
        "gpu_device_identifier": source.gpu_device_identifier,
        "gpu_compute_capability": source.gpu_compute_capability,
        "gpu_total_memory_mib": source.gpu_total_memory_mib,
        "cuda_visible_memory_mib": source.cuda_visible_memory_mib,
    }


def _acceleration_contract_payload(
    source: object,
    *,
    selected_backend: str,
    fallback_reason: Optional[str],
) -> Dict[str, object]:
    payload = _acceleration_snapshot_payload(source)
    payload["selected_ridge_backend"] = selected_backend
    payload["fallback_reason"] = fallback_reason
    return payload


def _positive_resource_count(value: object, *, name: str) -> int:
...
def _cuda_api_version(value: int) -> str:
    encoded = _positive_resource_count(value, name="CUDA API version")
    return f"{encoded // 1000}.{(encoded % 1000) // 10}"


def _nvidia_gpu_resource_snapshot(
    device_identifier: str,
) -> Tuple[Optional[str], Optional[int], Optional[str]]:
    if not isinstance(device_identifier, str) or not device_identifier.strip():
        return None, None, "NVIDIA device identifier is missing or malformed"
    identifier = device_identifier.strip()
    try:
        completed = subprocess.run(
            ...
            timeout=ACCELERATION_NVIDIA_SMI_TIMEOUT_S,
        )
    except subprocess.TimeoutExpired as exc:
        return (
            None,
            None,
            "nvidia-smi timed out for device "
            f"{identifier} after {exc.timeout} seconds",
        )
    except OSError as exc:
        return (
            None,
            None,
            f"nvidia-smi execution failed for device {identifier}: "
    ...
    if completed.returncode != 0:
        ...
        return (
            None,
            None,
            f"nvidia-smi for device {identifier} exited "
    ...
    if len(rows) != 1:
        return (
            None,
            None,
            f"nvidia-smi for device {identifier} returned an unexpected row count",
        )
    parts = tuple(part.strip() for part in rows[0].split(","))
    if len(parts) != 2 or not parts[0]:
        return None, None, f"nvidia-smi for device {identifier} returned malformed CSV"
    try:
        memory_mib = _positive_resource_count(
            int(parts[1]),
            name="nvidia-smi memory.total",
        )
    except ValueError:
        return (
            None,
            None,
            f"nvidia-smi for device {identifier} returned malformed memory.total",
        )
    return parts[0], memory_mib, None


def _installed_optional_dependency_version(name: str) -> Optional[str]:
    candidates = {
        "numba": ("numba",),
...
def _optional_dependency(name: str):
    try:
        ...
        version = _installed_optional_dependency_version(name)
        available = version is not None
        state = "import failed" if available else "is not installed"
        return (
            None,
            available,
            version,
            f"{name} {state}: {type(exc).__name__}: {exc}",
        )
    ...
    if not isinstance(version, str) or not version.strip():
        return (
            ...
            True,
            None,
            f"{name} import succeeded but its version is unavailable",
        )
    return module, True, version.strip(), None


def _cleanup_cupy_probe(cupy) -> Optional[str]:
    failures = []
    try:
        cupy.cuda.runtime.deviceSynchronize()
    except Exception as exc:
        failures.append(f"deviceSynchronize failed: {type(exc).__name__}: {exc}")
    memory_pool_getter = getattr(cupy, "get_default_memory_pool", None)
    if callable(memory_pool_getter):
        try:
            memory_pool = memory_pool_getter()
            free_all_blocks = getattr(memory_pool, "free_all_blocks", None)
            if callable(free_all_blocks):
                free_all_blocks()
        except Exception as exc:
            failures.append(
                f"memory-pool cleanup failed: {type(exc).__name__}: {exc}"
            )
    return "; ".join(failures) if failures else None


def _probe_cupy_runtime(cupy) -> Tuple[Dict[str, object], Optional[str]]:
    ...
    try:
        ...
        raw_identifier = cupy.cuda.runtime.deviceGetPCIBusId(0)
        device_identifier = (
            raw_identifier.decode("utf-8", errors="strict")
            if isinstance(raw_identifier, bytes)
            else str(raw_identifier)
        ).strip()
        if not device_identifier:
            raise RuntimeError("CUDA device PCI bus identifier is empty")
        nvidia_driver, physical_memory_mib, nvidia_reason = (
            _nvidia_gpu_resource_snapshot(device_identifier)
        )
        if nvidia_reason is not None:
            raise RuntimeError(nvidia_reason)
        payload = {
            "cuda_driver_version": _cuda_api_version(
                int(cupy.cuda.runtime.driverGetVersion())
            ),
            "cuda_runtime_version": _cuda_api_version(
                int(cupy.cuda.runtime.runtimeGetVersion())
            ),
            "nvidia_driver_version": nvidia_driver,
            "gpu_name": gpu_name,
            "gpu_device_identifier": device_identifier,
            "gpu_compute_capability": (
                f"{int(device['major'])}.{int(device['minor'])}"
            ),
            "gpu_total_memory_mib": physical_memory_mib,
            "cuda_visible_memory_mib": (
                int(device["totalGlobalMem"]) // (1024 * 1024)
            ),
        }
        values = cupy.arange(16, dtype=cupy.float64)
        incremented = values + 1
        expected = cupy.arange(1, 17)
        if not bool(cupy.all(incremented == expected)):
            raise RuntimeError("CuPy smoke result mismatch")
    except Exception as exc:
        payload = {}
        failure_reason = (
            "CuPy CUDA probe failed: "
            f"{type(exc).__name__}: {exc}"
        )
    finally:
        values = None
        incremented = None
        expected = None
        cleanup_reason = _cleanup_cupy_probe(cupy)
        if cleanup_reason is not None:
            payload = {}
            if failure_reason is None:
                failure_reason = f"CuPy cleanup failed: {cleanup_reason}"
            else:
                failure_reason = f"{failure_reason}; {cleanup_reason}"
    return payload, failure_reason


def _probe_acceleration_system_snapshot() -> AccelerationSystemSnapshot:
    numba, numba_available, numba_version, numba_reason = (
        _optional_dependency("numba")
    )
    cupy, cupy_available, cupy_version, cupy_reason = (
        _optional_dependency("cupy")
    )
    numba_compatible = numba is not None and numba_reason is None
    if numba_compatible:
        try:
            increment = numba.njit(cache=False)(lambda value: value + 1)
            if increment(1) != 2:
                raise RuntimeError("Numba smoke result mismatch")
        except Exception as exc:
            numba_compatible = False
            numba_reason = (
                "Numba njit smoke failed: "
                f"{type(exc).__name__}: {exc}"
            )
    cupy_payload: Dict[str, object] = {
        "cuda_driver_version": None,
        "cuda_runtime_version": None,
        "nvidia_driver_version": None,
        "gpu_name": None,
        "gpu_device_identifier": None,
        "gpu_compute_capability": None,
        "gpu_total_memory_mib": None,
        "cuda_visible_memory_mib": None,
    }
    cupy_compatible = cupy is not None and cupy_reason is None
    if cupy_compatible:
        observed_cupy_payload, observed_cupy_reason = _probe_cupy_runtime(cupy)
        if observed_cupy_reason is not None:
            cupy_compatible = False
            cupy_reason = observed_cupy_reason
        else:
            cupy_payload = observed_cupy_payload
    backends = tuple(
        sorted(
            (
                str(info.get("user_api", "")),
                str(info.get("internal_api", "")),
                str(info.get("prefix", "")),
                int(info.get("num_threads", 0)),
            )
            for info in threadpool_info()
        )
    )
    cpu_count = os.cpu_count()
    return AccelerationSystemSnapshot(
        ...
        numba_compatible=numba_compatible,
        numba_failure_reason=numba_reason,
        cupy_available=cupy_available,
        cupy_compatible=cupy_compatible,
        cupy_failure_reason=cupy_reason,
        cuda_driver_version=cupy_payload["cuda_driver_version"],
        cuda_runtime_version=cupy_payload["cuda_runtime_version"],
        nvidia_driver_version=cupy_payload["nvidia_driver_version"],
        gpu_name=cupy_payload["gpu_name"],
        gpu_device_identifier=cupy_payload["gpu_device_identifier"],
        gpu_compute_capability=cupy_payload["gpu_compute_capability"],
        gpu_total_memory_mib=cupy_payload["gpu_total_memory_mib"],
        cuda_visible_memory_mib=cupy_payload["cuda_visible_memory_mib"],
    )


def _validate_acceleration_snapshot(
    snapshot: AccelerationSystemSnapshot,
) -> None:
    if not isinstance(snapshot, AccelerationSystemSnapshot):
        raise TypeError("snapshot must be an AccelerationSystemSnapshot")
    if not isinstance(snapshot.python_version, str) or not snapshot.python_version:
        raise ValueError("python_version must be a non-empty string")
    _positive_resource_count(snapshot.cpu_count, name="cpu_count")
    if set(snapshot.dependency_versions) != set(ACCELERATION_DEPENDENCY_NAMES):
        raise ValueError("dependency_versions must contain the frozen dependency set")
    for name in ACCELERATION_DEPENDENCY_NAMES:
        version = snapshot.dependency_versions[name]
        if version is None and name in ("numba", "cupy"):
            continue
        if not isinstance(version, str) or not version.strip():
            raise ValueError(f"dependency version for {name} must be non-empty")
    if snapshot.dependency_versions["python"] != snapshot.python_version:
        raise ValueError("Python dependency version must match python_version")
    if not isinstance(snapshot.numba_available, bool) or not isinstance(
        snapshot.numba_compatible, bool
    ):
        raise TypeError("Numba availability flags must be booleans")
    if not isinstance(snapshot.cupy_available, bool) or not isinstance(
        snapshot.cupy_compatible, bool
    ):
        raise TypeError("CuPy availability flags must be booleans")
    if snapshot.numba_compatible and not snapshot.numba_available:
        raise ValueError("compatible Numba must be available")
    if snapshot.cupy_compatible and not snapshot.cupy_available:
        raise ValueError("compatible CuPy must be available")
    numba_version = snapshot.dependency_versions["numba"]
    cupy_version = snapshot.dependency_versions["cupy"]
    if snapshot.numba_available != (numba_version is not None):
        raise ValueError("Numba availability must match its dependency version")
    if snapshot.cupy_available != (cupy_version is not None):
        raise ValueError("CuPy availability must match its dependency version")
    if snapshot.numba_compatible:
        if snapshot.numba_failure_reason is not None:
            raise ValueError("compatible Numba cannot retain a failure reason")
    elif not isinstance(snapshot.numba_failure_reason, str) or not (
        snapshot.numba_failure_reason.strip()
    ):
        raise ValueError("incompatible Numba requires a failure reason")
    if snapshot.cupy_compatible:
        if snapshot.cupy_failure_reason is not None:
            raise ValueError("compatible CuPy cannot retain a failure reason")
    elif not isinstance(snapshot.cupy_failure_reason, str) or not (
        snapshot.cupy_failure_reason.strip()
    ):
        raise ValueError("incompatible CuPy requires a failure reason")
    cuda_observations = {
        ...
        "gpu_device_identifier": snapshot.gpu_device_identifier,
        "gpu_compute_capability": snapshot.gpu_compute_capability,
        "gpu_total_memory_mib": snapshot.gpu_total_memory_mib,
        "cuda_visible_memory_mib": snapshot.cuda_visible_memory_mib,
    }
    if not snapshot.cupy_available and any(
        value is not None for value in cuda_observations.values()
    ):
        raise ValueError("unavailable CuPy cannot retain CUDA observations")
    if not snapshot.numba_compatible:
        reason = snapshot.numba_failure_reason or "no compatibility reason recorded"
        raise RuntimeError(f"Numba is required for Stage B acceleration: {reason}")
    if set(snapshot.thread_environment) != set(
        FORMAL_STAGE_B_THREAD_ENVIRONMENT
    ):
        raise ValueError("thread_environment must contain exactly five variables")
    for name in FORMAL_STAGE_B_THREAD_ENVIRONMENT:
        if snapshot.thread_environment[name] != "1":
            raise ValueError(f"{name} must be exactly the string '1'")
    if not snapshot.threadpool_backends:
        raise ValueError("at least one numerical threadpool backend is required")
    for row in snapshot.threadpool_backends:
        if len(row) != 4 or any(not isinstance(value, str) for value in row[:3]):
            raise ValueError("threadpool backend rows must use the frozen schema")
        if _positive_resource_count(row[3], name="threadpool num_threads") != 1:
            raise ValueError("threadpool backends must each use exactly one thread")
    if snapshot.gpu_total_memory_mib is not None:
        _positive_resource_count(
            snapshot.gpu_total_memory_mib,
            name="gpu_total_memory_mib",
        )
    if snapshot.cuda_visible_memory_mib is not None:
        _positive_resource_count(
            snapshot.cuda_visible_memory_mib,
            name="cuda_visible_memory_mib",
        )
    if (
        snapshot.gpu_total_memory_mib is not None
        and snapshot.cuda_visible_memory_mib is not None
        and snapshot.cuda_visible_memory_mib > snapshot.gpu_total_memory_mib
    ):
        raise ValueError(
            "cuda_visible_memory_mib cannot exceed gpu_total_memory_mib"
        )
    if snapshot.cupy_compatible:
        if snapshot.dependency_versions["cupy"] is None:
            raise ValueError("compatible CuPy requires a dependency version")
        ...
        for name, value in required_gpu_strings.items():
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"compatible CuPy requires {name}")
        try:
            capability_parts = snapshot.gpu_compute_capability.split(".")
            if len(capability_parts) != 2:
                raise ValueError
            major, minor = (int(part) for part in capability_parts)
        except (AttributeError, TypeError, ValueError) as exc:
            raise ValueError("GPU compute capability must use major.minor") from exc
        if major <= 0 or minor < 0:
            raise ValueError("GPU compute capability must use positive major.minor")
        if snapshot.gpu_total_memory_mib is None:
            raise ValueError("compatible CuPy requires gpu_total_memory_mib")
        if snapshot.cuda_visible_memory_mib is None:
            raise ValueError(
                "compatible CuPy requires cuda_visible_memory_mib"
            )


def acceleration_backend_probe(
    *, snapshot: Optional[AccelerationSystemSnapshot] = None
) -> AccelerationBackendEvidence:
    """Probe and canonically lock the Stage B acceleration prerequisites."""

    observed = _probe_acceleration_system_snapshot() if snapshot is None else snapshot
    _validate_acceleration_snapshot(observed)
    if observed.cupy_compatible:
        selected_backend = "cupy"
        fallback_reason = None
    else:
        selected_backend = "numba"
        fallback_reason = observed.cupy_failure_reason
        if not isinstance(fallback_reason, str) or not fallback_reason.strip():
            fallback_reason = (
                "CuPy is unavailable or incompatible; selected required Numba fallback"
            )
        fallback_reason = fallback_reason.strip()
    contract = _acceleration_contract_payload(
        observed,
    ...
    return AccelerationBackendEvidence(
        selected_ridge_backend=selected_backend,
        fallback_reason=fallback_reason,
        python_version=observed.python_version,
        dependency_versions=observed.dependency_versions,
        cpu_count=observed.cpu_count,
        thread_environment=observed.thread_environment,
        threadpool_backends=observed.threadpool_backends,
        numba_available=observed.numba_available,
        numba_compatible=observed.numba_compatible,
        numba_failure_reason=observed.numba_failure_reason,
        cupy_available=observed.cupy_available,
        cupy_compatible=observed.cupy_compatible,
        cupy_failure_reason=observed.cupy_failure_reason,
        cuda_driver_version=observed.cuda_driver_version,
        cuda_runtime_version=observed.cuda_runtime_version,
        nvidia_driver_version=observed.nvidia_driver_version,
        gpu_name=observed.gpu_name,
        gpu_device_identifier=observed.gpu_device_identifier,
        gpu_compute_capability=observed.gpu_compute_capability,
        gpu_total_memory_mib=observed.gpu_total_memory_mib,
        cuda_visible_memory_mib=observed.cuda_visible_memory_mib,
...
def read_stack_trace(
    ...
    *,
    ...
    """Read one HDF5 stack or three-column DAT and build its positive branch."""

    source_path = Path(path)
    if source_path.suffix.lower() == ".dat":
        try:
            dat = read_dat_trace(str(source_path))
        ...
        time = np.asarray(dat.time_s, dtype=float)
        positive = np.asarray(dat.positive_lag, dtype=float)
        negative = np.asarray(dat.negative_lag_reversed, dtype=float)
        if (
            time.ndim != 1
            or positive.shape != time.shape
            or negative.shape != time.shape
            or time.size < 2
            ...
            or not np.allclose(
                ...
                float(dat.dt_s),
                rtol=0.0,
                atol=4096.0 * np.finfo(float).eps * max(1.0, float(dat.dt_s)),
            )
            or not math.isclose(
                float(time[0]),
                float(dat.dt_s),
                rel_tol=0.0,
                abs_tol=64.0 * np.finfo(float).eps * max(1.0, float(dat.dt_s)),
            )
        ):
            raise MeasurementError("invalid_dat_time_axis")
        return StackTrace(
            pair_name=pair_name,
            dt_s=float(dat.dt_s),
            maxlag_s=float(time[-1]),
            time_positive_s=np.concatenate(([0.0], time)),
            positive_lag=np.concatenate(([0.0], positive)),
            negative_lag_reversed=np.concatenate(([0.0], negative)),
            symmetric=np.concatenate(([0.0], 0.5 * (positive + negative))),
            branch_mismatch=float(
                np.linalg.norm(positive - negative)
                / max(
                    float(np.linalg.norm(positive) + np.linalg.norm(negative)),
...
def audit_input_inventory_and_lineage(
    ...
    *,
    ...
    dat_paths = tuple(sorted(root.glob("*.dat")))
    input_paths = stack_paths if stack_paths else dat_paths
    ...
    for path in input_paths:
        try:
            if path.suffix.lower() == ".dat":
                pair_name = path.stem
                trace = read_stack_trace(path, pair_name=pair_name)
                for target, key in (
                    (dt_counts, _distribution_key(trace.dt_s)),
                    (sample_counts, str(int(2 * trace.symmetric.size - 1))),
                    (maxlag_counts, _distribution_key(trace.maxlag_s)),
                ):
    ...
    return {
        "stack_root": str(root),
        "component": component,
        "stack_file_count": len(input_paths),
...
def iter_stack_tasks(
    ...
    *,
    ...
    dat_paths = tuple(sorted(Path(stack_root).glob("*.dat")))
    if dat_paths:
        for dat_path in dat_paths:
            try:
                source_code, receiver_code = dat_path.stem.split("__", 1)
                dat = read_dat_trace(str(dat_path))
            except (ValueError, OSError, IndexError):
                continue
            if not pair_passes_bbox(
                float(dat.lon_a), float(dat.lat_a), float(dat.lon_b), float(dat.lat_b),
                bbox=bbox, mode=bbox_mode,
            ):
                continue
            yield (
                str(dat_path), source_code, receiver_code,
                float(dat.lon_a), float(dat.lat_a), float(dat.lon_b), float(dat.lat_b),
            )
        return
...
def _validate_stage_c_evidence_structure(
    ...
    try:
        selected_pair_names = tuple(selection["selected_pair_names"])
        if (
            not selected_pair_names
            or any(
                not isinstance(name, str) or not name
                for name in selected_pair_names
            )
            or len(set(selected_pair_names)) != len(selected_pair_names)
        ):
            raise ValueError("selected pair archive is invalid")
        restored_budget = wang_ftan_validation.validate_archived_stage_b_budget(
            budget_payload=budget,
            benchmark_payload=benchmark,
            selected_pair_count=len(selected_pair_names),
            maximum_measurement_class_count=len(candidates),
            worker_count=int(benchmark["ftan_requested_worker_count"]),
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(
            "Stage B benchmark resource evidence is incomplete"
        ) from exc
    if (
        not isinstance(budget, dict)
        or budget.get("accepted") is not True
        or budget.get("status") != "accepted"
        or not isinstance(restored_budget, wang_ftan_validation.StageBBudgetResult)
...
def fit_stage_b_split_half_reference(
    *,
    ...
    prepared = prepare_reference_objective(observations, periods_s=periods)
    solutions = []
    for start in starts:
...
_LINUX_MEMAVAILABLE_PATTERN = re.compile(
    r"^MemAvailable:[ \t]+([0-9]+)[ \t]+kB[ \t]*$"
)


def _available_memory_observation(
    *,
    platform_name: Optional[str] = None,
    linux_meminfo_text: Optional[str] = None,
    darwin_vm_stat_text: Optional[str] = None,
    sysconf_reader=os.sysconf,
) -> Dict[str, object]:
    """Return one explicit, canonical available-memory observation."""

    observed_platform = (
        platform.system() if platform_name is None else str(platform_name)
    )
    if not observed_platform:
        raise ValueError("available-memory platform is unavailable")
    if observed_platform == "Linux":
        text = (
            Path("/proc/meminfo").read_text(encoding="ascii")
            if linux_meminfo_text is None
            else str(linux_meminfo_text)
        )
        candidates = [
            line
            for line in text.splitlines()
            if line.partition(":")[0].strip() == "MemAvailable"
            or line.lstrip().startswith("MemAvailable")
        ]
        if len(candidates) != 1:
            raise ValueError(
                "Linux /proc/meminfo must contain exactly one MemAvailable"
            )
        match = _LINUX_MEMAVAILABLE_PATTERN.fullmatch(candidates[0])
        if match is None:
            raise ValueError(
                "Linux MemAvailable must be one non-negative integer in kB"
            )
        raw_value = int(match.group(1))
        if raw_value <= 0:
            raise ValueError("Linux MemAvailable must be positive")
        bytes_per_raw_unit = 1024
        source = "/proc/meminfo"
        raw_field = "MemAvailable"
        raw_unit = "kB"
    else:
        try:
            if darwin_vm_stat_text is not None:
                raise ValueError("use injected vm_stat evidence")
            raw_value = int(sysconf_reader("SC_AVPHYS_PAGES"))
            bytes_per_raw_unit = int(sysconf_reader("SC_PAGE_SIZE"))
            source = "os.sysconf"
            raw_field = "SC_AVPHYS_PAGES"
        except (AttributeError, OSError, TypeError, ValueError) as exc:
            if observed_platform != "Darwin":
                raise ValueError(
                    "non-Linux available-memory fallback is unavailable"
                ) from exc
            if darwin_vm_stat_text is None:
                completed = subprocess.run(
                    ["vm_stat"],
                    check=False,
                    text=True,
                ...
                if completed.returncode != 0:
                    raise ValueError(
                        "Darwin vm_stat available-memory fallback failed"
                    ) from exc
                darwin_vm_stat_text = completed.stdout
            header = re.search(
                r"page size of ([0-9]+) bytes",
                darwin_vm_stat_text,
            )
            free_rows = re.findall(
                r"^Pages free:[ \t]+([0-9]+)\.[ \t]*$",
                darwin_vm_stat_text,
                flags=re.MULTILINE,
            )
            if header is None or len(free_rows) != 1:
                raise ValueError(
                    "Darwin vm_stat available-memory evidence is incomplete"
                ) from exc
            raw_value = int(free_rows[0])
            bytes_per_raw_unit = int(header.group(1))
            source = "vm_stat"
            raw_field = "Pages free"
        if raw_value <= 0 or bytes_per_raw_unit <= 0:
            raise ValueError(
                "non-Linux available-memory fallback must be positive"
            )
        raw_unit = "pages"
    evidence = wang_ftan_validation.MemoryAvailabilityEvidence(
        platform=observed_platform,
        source=source,
        raw_field=raw_field,
        raw_value=raw_value,
        raw_unit=raw_unit,
        bytes_per_raw_unit=bytes_per_raw_unit,
        canonical_field="mem_available_bytes",
        canonical_value=raw_value * bytes_per_raw_unit,
        canonical_unit="bytes",
    )
    return wang_ftan_validation._stage_b_jsonable(evidence)


def _available_memory_bytes() -> int:
    """Backward-compatible canonical value for non-sampled callers."""

    return int(_available_memory_observation()["canonical_value"])


@dataclass
class ResourceSamplerState:
    """Mutable state for one parent and one unchanged live worker pool."""

    parent_pid: int
    worker_pids: Tuple[int, ...]
    process_identities: Tuple[
        wang_ftan_validation.ProcessIdentityEvidence,
        ...,
    ]
    lifecycle_nonce: str
    interval_s: float
    ...
    minimum_complete_mem_available_bytes: Optional[int] = None
    minimum_mem_available_observation: Optional[Dict[str, object]] = None
    peak_aggregate_rss_bytes: int = 0
    peak_sampled_monotonic_s: Optional[float] = None
    peak_rss_by_pid: Dict[int, int] = field(default_factory=dict)
    sampling_failure: Optional[str] = None

    def __post_init__(self) -> None:
        raw_parent = self.parent_pid
        raw_workers = tuple(self.worker_pids)
        self.parent_pid = int(self.parent_pid)
        self.worker_pids = tuple(int(value) for value in raw_workers)
        identities = []
        for value in self.process_identities:
            identity = value
            if not isinstance(
                identity,
                wang_ftan_validation.ProcessIdentityEvidence,
            ):
                identity = wang_ftan_validation.ProcessIdentityEvidence(
                    **dict(value)
                )
            identities.append(identity)
        self.process_identities = tuple(identities)
        self.interval_s = float(self.interval_s)
        self.started_monotonic_s = float(self.started_monotonic_s)
        if (
            isinstance(raw_parent, (bool, np.bool_))
            or not isinstance(raw_parent, (int, np.integer))
            or self.parent_pid <= 0
            or not self.worker_pids
            or any(
                isinstance(value, (bool, np.bool_))
                or not isinstance(value, (int, np.integer))
                for value in raw_workers
            )
            or any(value <= 0 for value in self.worker_pids)
            or len(set(self.worker_pids)) != len(self.worker_pids)
            or self.parent_pid in self.worker_pids
            or not np.isfinite(self.interval_s)
            or self.interval_s <= 0
            or not np.isfinite(self.started_monotonic_s)
            or tuple(value.pid for value in self.process_identities)
            != (self.parent_pid,) + self.worker_pids
        ):
            raise ValueError("resource sampler lifecycle inputs are invalid")
        self.lifecycle_id = (
            wang_ftan_validation.derive_pool_resource_lifecycle_id(
                process_identities=self.process_identities,
...
def _record_complete_pool_resource_sample(
    state: ResourceSamplerState,
    *,
    lifecycle_id: str,
    parent_pid: int,
    worker_pids: Sequence[int],
    process_identities_before: Sequence[Mapping[str, object]],
    process_identities_after: Sequence[Mapping[str, object]],
    rss_by_pid: Mapping[int, int],
    memory_observation: Mapping[str, object],
    sampled_monotonic_s: float,
) -> None:
    """Record one all-process RSS/MemAvailable sample for one lifecycle."""

    if not isinstance(state, ResourceSamplerState):
        raise ValueError("resource sampler lifecycle state is invalid")
    parent = int(parent_pid)
    raw_workers = tuple(worker_pids)
    ...
    try:
        before = tuple(
            ...
            else wang_ftan_validation.ProcessIdentityEvidence(**dict(value))
            for value in process_identities_before
        )
        after = tuple(
            value
            if isinstance(
            ...
            for value in process_identities_after
        )
    except (TypeError, ValueError) as exc:
        raise ValueError("resource sample process identity is invalid") from exc
    if (
        str(lifecycle_id) != state.lifecycle_id
        or parent != state.parent_pid
        or workers != state.worker_pids
        or len(set(workers)) != len(workers)
        or before != state.process_identities
        or after != state.process_identities
    ):
        raise ValueError("resource sample lifecycle/PID identity drifted")
    raw_rss = dict(rss_by_pid)
    try:
        normalized_rss = {
            int(process_id): int(value)
            for process_id, value in raw_rss.items()
        }
    except (TypeError, ValueError) as exc:
        raise ValueError("resource sample RSS evidence is invalid") from exc
    expected_pids = {parent, *workers}
    if (
        len(normalized_rss) != len(raw_rss)
        or set(normalized_rss) != expected_pids
        or any(
            isinstance(value, (bool, np.bool_))
            or not isinstance(value, (int, np.integer))
            or int(value) <= 0
            for value in raw_rss.values()
        )
    ):
        raise ValueError("resource sample RSS PID set is incomplete")
    sampled = float(sampled_monotonic_s)
    if not np.isfinite(sampled) or sampled < state.started_monotonic_s:
        raise ValueError("resource sample lifecycle timestamp is invalid")
    try:
        available = wang_ftan_validation.MemoryAvailabilityEvidence(
            **dict(memory_observation)
        )
    except (TypeError, ValueError) as exc:
        raise ValueError(
            "available-memory observation is incomplete"
        ) from exc
    canonical_value = available.canonical_value
    aggregate = int(sum(normalized_rss.values()))
    state.complete_sample_count += 1
    if (
        state.minimum_complete_mem_available_bytes is None
        or canonical_value < state.minimum_complete_mem_available_bytes
    ):
        state.minimum_complete_mem_available_bytes = canonical_value
        state.minimum_mem_available_observation = (
            wang_ftan_validation._stage_b_jsonable(available)
        )
    if aggregate > state.peak_aggregate_rss_bytes:
        state.peak_aggregate_rss_bytes = aggregate
        state.peak_sampled_monotonic_s = sampled
        state.peak_rss_by_pid = dict(normalized_rss)


def _finalize_pool_resource_sampler_state(
    state: ResourceSamplerState,
    *,
    ended_monotonic_s: float,
) -> Dict[str, object]:
    """Freeze and validate complete lifecycle evidence for archival."""

    ended = float(ended_monotonic_s)
    if state.sampling_failure:
        raise RuntimeError(
            f"resource sampler rejected an incomplete sample: "
            f"{state.sampling_failure}"
        )
    if state.complete_sample_count <= 0:
        raise RuntimeError("resource sampler captured no complete sample")
    try:
        evidence = wang_ftan_validation.PoolResourceEvidence(
            lifecycle_id=state.lifecycle_id,
            lifecycle_nonce=state.lifecycle_nonce,
            parent_pid=state.parent_pid,
            worker_pids=state.worker_pids,
            process_identities=state.process_identities,
            started_monotonic_s=state.started_monotonic_s,
            ended_monotonic_s=ended,
            complete_sample_count=state.complete_sample_count,
            minimum_mem_available_observation=(
                state.minimum_mem_available_observation
            ),
            minimum_complete_mem_available_bytes=(
                state.minimum_complete_mem_available_bytes
            ),
            peak_aggregate_rss_bytes=state.peak_aggregate_rss_bytes,
            peak_sampled_monotonic_s=state.peak_sampled_monotonic_s,
            peak_rss_by_pid=state.peak_rss_by_pid,
        )
    except (TypeError, ValueError) as exc:
        raise RuntimeError(
            "resource sampler lifecycle evidence is incomplete"
        ) from exc
    return wang_ftan_validation._stage_b_jsonable(evidence)
...
def _read_process_identity(
    ...
    *,
    platform_name: Optional[str] = None,
    linux_proc_stat_text: Optional[str] = None,
    darwin_ps_text: Optional[str] = None,
) -> Dict[str, object]:
    """Read an OS-backed process start identity that detects PID reuse."""
    ...
    if observed_platform == "Linux":
        text = (
            Path(f"/proc/{process_id}/stat").read_text(encoding="ascii")
            if linux_proc_stat_text is None
            else str(linux_proc_stat_text)
        )
        closing = text.rfind(")")
        prefix = text[:closing].split(" ", 1) if closing >= 0 else ()
        tail = text[closing + 1 :].split() if closing >= 0 else ()
        if (
            len(prefix) != 2
            or int(prefix[0]) != process_id
            or len(tail) < 20
        ):
            raise ValueError("Linux process starttime evidence is incomplete")
        try:
            starttime_ticks = int(tail[19])
        except (TypeError, ValueError) as exc:
            raise ValueError(
                "Linux process starttime evidence is invalid"
            ) from exc
        evidence = wang_ftan_validation.ProcessIdentityEvidence(
            pid=process_id,
            platform="Linux",
            source="/proc/<pid>/stat:starttime",
            starttime_ticks=starttime_ticks,
            native_start_time=str(starttime_ticks),
        )
    elif observed_platform == "Darwin":
        if darwin_ps_text is None:
            try:
                completed = subprocess.run(
                ...
                raise ProcessLookupError(process_id) from exc
            if completed.returncode != 0:
                raise ProcessLookupError(process_id)
            darwin_ps_text = completed.stdout
        rows = [row.strip() for row in darwin_ps_text.splitlines() if row.strip()]
        if len(rows) != 1:
            raise ValueError("Darwin process start identity is incomplete")
        try:
            datetime.strptime(rows[0], "%a %b %d %H:%M:%S %Y")
        except ValueError as exc:
            raise ValueError("Darwin process start identity is invalid") from exc
        evidence = wang_ftan_validation.ProcessIdentityEvidence(
            pid=process_id,
            platform="Darwin",
            source="ps:lstart",
            starttime_ticks=None,
            native_start_time=rows[0],
        )
    else:
        raise RuntimeError(
            f"unsupported process identity platform: {observed_platform}"
        )
    return wang_ftan_validation._stage_b_jsonable(evidence)
...
def _start_pool_rss_sampler(
    *,
    ...
    worker_pids: Sequence[int],
    started_monotonic_s: float,
    lifecycle_nonce: Optional[str] = None,
    interval_s: float = 0.02,
    rss_reader=_read_process_rss_bytes,
    memory_reader=_available_memory_observation,
    identity_reader=_read_process_identity,
):
    """Sample aggregate RSS and MemAvailable for one live process pool."""
    ...
    state = ResourceSamplerState(
        ...
        lifecycle_nonce=(
            ...
            if lifecycle_nonce is None
            else lifecycle_nonce
        ),
        interval_s=interval,
    ...
    def sample_once() -> None:
        ...
        for process_id in (parent,) + workers:
            rss_by_pid[process_id] = rss_reader(process_id)
        sampled = time.perf_counter()
        identities_after = tuple(
            identity_reader(process_id)
            for process_id in (parent,) + workers
        )
        _record_complete_pool_resource_sample(
            state,
            lifecycle_id=state.lifecycle_id,
            parent_pid=parent,
            worker_pids=workers,
            process_identities_before=identities_before,
            process_identities_after=identities_after,
            rss_by_pid=rss_by_pid,
            memory_observation=memory_reader(),
            sampled_monotonic_s=sampled,
        )
    ...
    def run() -> None:
        try:
            ...
            state.sampling_failure = f"{type(exc).__name__}: {exc}"
            stop.set()
...
def _stop_pool_resource_sampler(
    stop: threading.Event,
    sampler: threading.Thread,
    *,
    timeout_s: float = 5.0,
) -> None:
    """Stop and join one sampler, rejecting incomplete cleanup."""

    stop.set()
    sampler.join(timeout=timeout_s)
    if sampler.is_alive():
        raise RuntimeError("resource sampler cleanup failure: thread is alive")


def _shutdown_stage_b_benchmark_pool(pool, *, terminate: bool) -> None:
    """Close or terminate one pool and prove every worker was reaped."""

    pool.terminate() if terminate else pool.close()
    pool.join()
    lingering = tuple(
        int(process.pid) for process in pool._pool if process.is_alive()
    )
    if lingering:
        raise RuntimeError(
            f"benchmark pool cleanup failure: live worker PIDs {lingering}"
        )
...
def _execute_stage_b_benchmark_pool(
    ...
    *,
    ...
    try:
        ...
        try:
            _shutdown_stage_b_benchmark_pool(pool, terminate=True)
        except BaseException as cleanup_error:
            raise RuntimeError(
                f"benchmark startup cleanup failure: {cleanup_error}"
            ) from startup_error
        raise
    try:
        ...
        cleanup_errors = []
        try:
            _stop_pool_resource_sampler(stop, sampler)
        except BaseException as exc:
        ...
        if cleanup_errors:
            raise RuntimeError(
                "benchmark cleanup failure: " + "; ".join(cleanup_errors)
            ) from work_error
        raise
    try:
        live_worker_pids = tuple(
            sorted(
                int(process.pid)
        ...
        if live_worker_pids != worker_pids:
            memory.sampling_failure = "worker PID set drifted"
        _stop_pool_resource_sampler(stop, sampler)
        ended = time.perf_counter()
        resource_evidence = _finalize_pool_resource_sampler_state(
            memory,
            ended_monotonic_s=ended,
        )
        ...
        try:
            ...
            raise RuntimeError(
                f"benchmark cleanup failure: {cleanup_error}"
            ) from evidence_error
        raise
    _shutdown_stage_b_benchmark_pool(pool, terminate=False)
    return {
        "results": results,
        "creator_pid": creator_pid,
        "worker_pids": worker_pids,
...
def _evaluate_stage_b_fit_benchmark_job(job: Dict[str, object]):
    ...
    row = dict(job)
    started = time.perf_counter()
    prepared = prepare_reference_objective(
        context["observations"],
        periods_s=context["periods"],
    )
    objective = lambda candidate: prepared_reference_objective_value_and_gradient(
        candidate,
        prepared,
...
def _run_stage_b_benchmark_workload(*, max_workers: int) -> Dict[str, object]:
    ...
    return {
        ...
        "ftan_resource_evidence": dict(ftan_pool["resource_evidence"]),
        "fit_resource_evidence": dict(fit_pool["resource_evidence"]),
