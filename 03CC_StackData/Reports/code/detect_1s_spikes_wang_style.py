...
PAPER_PANEL_FIGSIZE = (5.4, 4.0)


def paper_panel_figure_size_inches() -> tuple[float, float]:
    return PAPER_PANEL_FIGSIZE


def save_panel_npz(path: Path, **arrays) -> None:
...
def make_wang_figure3a_plot(output, time, stacks, counts, bin_width, zoom_center_s=0.5, zoom_width_s=1.0):
    ...
    fig, ax = plt.subplots(figsize=paper_panel_figure_size_inches(), constrained_layout=True)
    for index in valid:
    ...
    save_panel_npz(
        output / "wang_figure3a_panel_data.npz",
        panel=np.array("a"),
        time=np.asarray(time, dtype=float),
        ...
        bin_width_km=np.array(float(bin_width)),
        diagnostic_window_s=np.array(0.8),
        display_amplitude_scale=np.array(0.55),
    )
    plot_distance_bin_panel(
        output_path=output / "wang_figure3a_style_diagnostic_residual.png",
        time=time,
        data=diagnostic,
        valid=valid,
        bin_width=bin_width,
        panel_label="(a)",
        use_diagnostic_guides=True,
...
def plot_distance_bin_panel(
    *,
    ...
    bin_width: float,
    panel_label: str,
    marker_phase_s: float | None = None,
    marker_seconds: range | None = None,
    use_diagnostic_guides: bool = False,
    amplitude_scale: float = 0.42,
    line_width: float = 0.45,
) -> None:
    fig, ax = plt.subplots(figsize=paper_panel_figure_size_inches(), constrained_layout=True)
    for index in valid:
        if not np.all(np.isfinite(data[index])):
            continue
        distance = (index + 0.5) * bin_width
        ax.plot(time, distance + data[index] * bin_width * amplitude_scale, color="#111827", lw=line_width)
    if marker_seconds is not None and marker_phase_s is not None:
        for second in marker_seconds:
            ax.axvline(second + marker_phase_s, color="#dc2626", lw=0.35, alpha=0.25)
    elif use_diagnostic_guides:
    ...
    ax.set_xlabel("Time (sec)")
    ax.set_ylabel("Distance (km)")
    ax.text(0.01, 1.02, panel_label, transform=ax.transAxes, ha="left", va="bottom", fontsize=10)
    ax.tick_params(top=True, right=True, length=3, width=0.8)
...
def make_template_subtraction_plots(output, time, stacks, counts, examples, bin_width, template_source="coherent"):
    ...
    plot_distance_bin_panel(
        output_path=output / "distance_bin_wiggle_before_only.png",
        time=time,
        data=normalized,
        valid=valid,
        bin_width=bin_width,
        panel_label="(a)",
        marker_phase_s=best_phase["phase_s"],
        marker_seconds=range(1, 16),
    )
    plot_distance_bin_panel(
        output_path=output / "distance_bin_wiggle_after_only.png",
        time=time,
        data=corrected_bins,
        valid=valid,
        bin_width=bin_width,
    ...
    save_panel_npz(
        output / "distance_bin_wiggle_panel_data.npz",
        panel=np.array("c"),
        time=np.asarray(time, dtype=float),
        counts=np.asarray(counts, dtype=int),
        valid=np.asarray(valid, dtype=int),
        normalized_before=np.asarray(normalized, dtype=float),
        corrected_after=np.asarray(corrected_bins, dtype=float),
        coherent_before=np.asarray(before, dtype=float),
        coherent_after=np.asarray(after, dtype=float),
        template_offsets_s=np.asarray(offsets, dtype=float),
        template_amplitude=np.asarray(template, dtype=float),
        best_phase_s=np.array(float(best_phase["phase_s"])),
        bin_width_km=np.array(float(bin_width)),
        display_amplitude_scale=np.array(0.42),
    )
